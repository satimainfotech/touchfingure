...
class Staff extends CI_Controller
{
    ...
    function admins($para1 = '', $para2 = '')
    {
        ...
        if ($para1 == 'do_add') {
            ...
            $this->load->view('back/abdaily/admins/admin_edit', $page_data);
        } elseif ($para1 == "update") {
            $data['name'] = $this->input->post('name');
			$password = $this->input->post('password');
            $data['password'] = sha1($password);
            ...
            $page_data['all_admins'] = $this->db->get('admin')->result_array();
            $this->load->view('back/abdaily/admins/admin_list', $page_data);
        } elseif ($para1 == 'view') {
            $page_data['admin_data'] = $this->db->get_where('admin', array('admin_id' => $para2))->result_array();
            $this->load->view('back/abdaily/admins/admin_view', $page_data);
        } elseif ($para1 == 'add') {
			$this->load->view('back/abdaily/admins/admin_add');
        }
    }
    ...
    function role($para1 = '', $para2 = '')
    {
        ...
        if ($para1 == 'do_add') {
            ...
            $this->load->view('back/abdaily/role/role_list', $page_data);
        } elseif ($para1 == 'view') {
            $page_data['role_data'] = $this->db->get_where('role', array('role_id' => $para2))->result_array();
            $this->load->view('back/abdaily/role/role_view', $page_data);
        } elseif ($para1 == 'add') {
            $page_data['all_permissions'] = $this->db->order_by('permission_id','asc')->get_where('permission',array('show' => 'yes'))->result_array();
            $this->load->view('back/abdaily/role/role_add', $page_data);
        } else if ($para1 == 'edit') {
            $page_data['all_permissions'] = $this->db->order_by('permission_id','asc')->get_where('permission',array('show' => 'yes'))->result_array();
            $page_data['role_data']       = $this->db->get_where('role', array('role_id' => $para2))->result_array();
            $this->load->view('back/abdaily/role/role_edit', $page_data);
        } else {
