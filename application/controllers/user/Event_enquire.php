...
class Event_enquire extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->database();
        $this->load->model('event_enquire_model');
    }
	...
	public function index()
    {
		...
		if ($this->session->userdata('admin_login') == 'yes') {
			...
			$count_data = $this->event_enquire_model->get_total_event_enquire_data_count($category,$sub_category,$product_name,$use_from_date,$use_to_date,$our_range);
			
			$config = array();		
			$config['total_rows'] = count($count_data);
			$config['base_url'] = base_url() . "admin/event_enquire".$searchurl;
			$config['per_page'] = 20;
			...
			$data['all_event_enquire'] = $this->event_enquire_model->get_total_event_enquire_data($category,$sub_category,$product_name,$use_from_date,$use_to_date,$our_range,
			...
			$data['page_id'] = $page;
			$data['page_name'] = "event_enquire/event_enquire";
            $data['page_name_link'] = "event_enquire";
	...
	public function stallbook()
    {
		...
		if ($this->session->userdata('admin_login') == 'yes') {
			...
			$count_data = $this->event_enquire_model->get_total_event_enquire_data_stall_count($category,$sub_category,$product_name,$use_from_date,$use_to_date,$our_range);
			
			$config = array();		
			$config['total_rows'] = count($count_data);
			$config['base_url'] = base_url() . "admin/event_enquire/stallbook".$searchurl;
			$config['per_page'] = 20;
			$config['uri_segment'] = '3';
			$config['page_query_string']= TRUE;
			$config['query_string_segment'] = "page";
			...
			$data['all_event_enquire'] = $this->event_enquire_model->get_total_event_enquire_stall_data($category,$sub_category,$product_name,$use_from_date,$use_to_date,$our_range,
			$config["per_page"],$page);
			...
			$data['total_rows'] = $config["total_rows"];
			$data['page_id'] = $page;
			$data['page_name'] = "event_enquire/stallbook";
            $data['page_name_link'] = "event_enquire/stallbook";
            $this->load->view('back/admin/index', $data);
        } else {
            $data['control'] = "admin";
            $this->load->view('back/admin/login',$data);
	...
	function event_enquires($para1 = '', $para2 = '', $para3 = '')
    {
        if (!$this->crud_model->admin_permission('product')) {
            redirect(base_url() . 'admin');
        }
	...
	function event_enquire_view(){
		...
		if ($this->session->userdata('admin_login') == 'yes') {
			...
			$page_data['event_enquire_data'] = $this->event_enquire_model->get_event_enquire_details($product_enq_id,$product_enq_token);
			$page_data['product_enquire_id'] = $product_enq_id;
			$page_data['page_id'] = $page_id;
            $page_data['page_name'] = "event_enquire/event_enquire_view";
            $page_data['page_name_link'] = "event_enquire";
	...
	public function excel_export()
	{
		...
		$product_data = $this->event_enquire_model->get_exal_product();
